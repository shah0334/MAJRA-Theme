...
function custom_vc_container() {
    vc_map(array(
        ...
        'js_view' => 'VcColumnView', // Enables WPBakery’s drag & drop
...
// Majra Clickable Slider
function vc_majra_clickable_slider() {
    if (!defined('WPB_VC_VERSION')) {
        return;
    }

    vc_map(array(
        'name'        => __('Majra Image Clickable Slider', 'text-domain'),
        'base'        => 'majra_clickable_slider',
        'description' => __('A customized Majra slider with multiple clickable slides', 'text-domain'),
        'category'    => __('Custom Elements', 'text-domain'),
        'icon'        => 'icon-wpb-images-carousel',
        'params'      => array(
            array(
                ...
                'heading'     => __('Slides', 'text-domain'),
                'param_name'  => 'slides',
                'description' => __('Add multiple slides with images and link.', 'text-domain'),
                'params'      => array(
                    array(
                        'type'       => 'attach_image',
                        'heading'    => __('Image', 'text-domain'),
                    ...
                    array(
                        ...
                        'param_name' => 'link',
                        'description'=> __('Enter the slide link', 'text-domain'),
                    ),
                ),
            ),
        ),
...
add_action('vc_before_init', 'vc_majra_clickable_slider');

function render_majra_clickable_slider($atts) {
    extract(shortcode_atts(array(
        'slides' => '', // This will store multiple slides
    ), $atts));
    ...
    ?>
                    <a class="swiper-slide d-flex" href="<?php echo $link; ?>">
                        <img src="<?php echo $image_url; ?>" class="d-block w-100 h-auto mx-auto" alt="...">
                    </a>
    <?php
                }
    ?>
...
add_shortcode('majra_clickable_slider', 'render_majra_clickable_slider');


// Majra Advance Slider
function vc_majra_advance_slider() {
    if (!defined('WPB_VC_VERSION')) {
        return;
    }

    vc_map(array(
        'name'        => __('Majra Advance Slider', 'text-domain'),
        'base'        => 'majra_advance_slider',
        'description' => __('A customized Majra slider with multiple features', 'text-domain'),
        'category'    => __('Custom Elements', 'text-domain'),
        'icon'        => 'icon-wpb-images-carousel',
        'params'      => array(
            array(
                ...
                'params'      => array(
                    ...
                    array(
                        'type'       => 'textfield',
                        'heading'    => __('Image/Video file URL', 'text-domain'),
                        'param_name' => 'media_url',
                        'description'=> __('Please enter direct URL of the uploaded image/video ', 'text-domain'),
                    ),
                    array(
                        'type'        => 'dropdown',
                        'heading'     => __('Media Type', 'text-domain'),
                        'description'=> __('Select for which media type link belongs to', 'text-domain'),
                    ...
                    array(
                        ...
                        'heading'    => __('Link', 'text-domain'),
                        'param_name' => 'link',
                        'description'=> __('Enter the link to open when clicked on slide', 'text-domain'),
                    ),
                    array(
                        'type'        => 'dropdown',
                        'heading'     => __('Link Target', 'text-domain'),
                        'description'=> __('Select where to open the link', 'text-domain'),
                    ...
                    array(
                        ...
                        'heading'    => __('Content', 'text-domain'),
                        'param_name' => 'content',
                        'description' => __('Content to display over slide.', 'text-domain'),
                    ),
                ),
            ),
            array(
                'type'       => 'checkbox',
                'heading'    => __('Enable slider dots', 'text-domain'),
                'param_name' => 'enable_dots',
                'description'=> __('Show slider dots', 'text-domain'),
                'value'       => array(__(' Yes', 'text-domain') => 'yes'),
            ),
            array(
                'type'       => 'checkbox',
                'heading'    => __('Enable slider arrows', 'text-domain'),
                'param_name' => 'enable_arrows',
                'description'=> __('Show slider arrows', 'text-domain'),
                'value'       => array(__(' Yes', 'text-domain') => 'yes'),
            ),
            array(
                'type'       => 'checkbox',
                'heading'    => __('Enable slider timer', 'text-domain'),
                'param_name' => 'enable_timer',
                'description'=> __('Show slider timer', 'text-domain'),
                'value'       => array(__(' Yes', 'text-domain') => 'yes'),
            ),
            array(
                'type'       => 'textfield',
                'heading'    => __('Slide change duration (Seconds)', 'text-domain'),
                'param_name' => 'slider_timeout',
                'description'=> __('Enter slider change duration. Default: 10 seconds', 'text-domain'),
            ),
            array(
                'type'       => 'textfield',
                'heading'    => __('Slider height', 'text-domain'),
                'param_name' => 'height',
                'description'=> __('Set slider height. E.g 100vh, 50vh, 500px, 300px, 100px', 'text-domain'),
                'value'      => '',
            ),
        ),
    ));
}
add_action('vc_before_init', 'vc_majra_advance_slider');

function render_majra_advance_slider($atts) {
    extract(shortcode_atts(array(
        'slides' => '', // This will store multiple slides
        'enable_dots' => '',
        'enable_arrows' => '',
    ...
    $enable_dots    = (!empty($atts['enable_dots']) && $atts['enable_dots'] === 'yes') ? true : false;
    $enable_arrows  = (!empty($atts['enable_arrows']) && $atts['enable_arrows'] === 'yes') ? true : false;
    $enable_timer   = (!empty($atts['enable_timer']) && $atts['enable_timer'] === 'yes') ? true : false;
    $slide_timeout  = (int)$atts['slider_timeout'];
    $height         = isset($atts['height']) ? esc_attr($atts['height']) : '100%';
    // Ensure a valid positive number with a default value of 10
    if ($slide_timeout <= 0) {
        $slide_timeout = 3;
    }
    // Convert JSON slides into an array
    ...
    $slider_id = 'majra-advance-slider-' . uniqid();

    ob_start();
    ?>
        <div class="position-relative majra-advance-slider">
            <div class="swiper swiper-dark <?php echo $slider_id; ?>" style="height: <?php echo $height; ?>">
            <div class="swiper-wrapper text-white">
    <?php
                foreach ($slides as $x => $slide) {
                    $content   = isset($slide['content']) ? wpautop($slide['content']) : '';

                    $media_url  = isset($slide['media_url']) ? esc_url($slide['media_url']) : '';
                    $media_type = isset($slide['media_type']) ? esc_attr($slide['media_type']) : 'img';

                    $link   = isset($slide['link']) ? esc_url($slide['link']) : '';
                    $target = isset($slide['link_target']) ? esc_attr($slide['link_target']) : '_self';

                    if( !empty($link) ){
                        echo '<a class="swiper-slide h-auto d-flex" href="'.$link.'" target="'.$target.'">';
                    }else{
                        echo '<div class="swiper-slide h-auto d-flex">';
                    }

                    if( $media_type == 'img' ){
                        echo '<img src="'.$media_url.'" class="d-block w-100 h-auto mx-auto" alt="...">';
                    }else{
                        echo '<video playsinline autoplay preload loop muted src="'.$media_url.'" class="d-block w-100 h-auto mx-auto"></video>';
                    }
                    if( !empty($content) ){
                        echo '<div class="content"><div class="container">'.$content.'</div></div>;';
    ...
    ?>
            ...
            <?php
                ...
                if( $enable_timer ){
                    echo '<div class="autoplay-indicator">
                        <svg viewBox="0 0 48 48">
                            <circle cx="24" cy="24" r="20"></circle>
                        </svg>
                        <span></span>
    ...
    <style>
        .majra-advance-slider .swiper-slide img,
        .majra-advance-slider .swiper-slide video {
            object-fit: cover; /* Prevents distortion */
        }
        .majra-advance-slider .swiper-button-next, .swiper-rtl .swiper-button-prev{
            right: 20px;
        }
        .majra-advance-slider .swiper-button-prev, .swiper-rtl .swiper-button-next{
            left: 20px;
        }
        .majra-advance-slider .swiper-pagination{
            left: 20px;
            bottom: 20px
        }
        .majra-advance-slider .content{
            position: absolute;
            top: 50%;
            left: 50%;
        ...
        .majra-advance-slider .autoplay-indicator {
            position: absolute;
            right: 20px;
            bottom: 20px;
            z-index: 10;
        ...
        .majra-advance-slider .autoplay-indicator svg {
            --progress: 0;
            position: absolute;
            left: 0;
            top: 0px;
        ...
        @media (max-width:640px){
            .majra-advance-slider > div{
                height: auto !important;
            }
            .majra-advance-slider .head-h1{
    ...
    <script>
        document.addEventListener("DOMContentLoaded", function() {
            const progressCircle = document.querySelector(".<?php echo esc_attr($slider_id); ?> .autoplay-indicator svg");
            const progressContent = document.querySelector(".<?php echo esc_attr($slider_id); ?> .autoplay-indicator span");
            ...
            new Swiper(".<?php echo esc_attr($slider_id); ?>", {
                ...
                spaceBetween: 0,
                autoplay: {
                    delay: 1000 * <?php echo $slide_timeout; ?>,
...
add_shortcode('majra_advance_slider', 'render_majra_advance_slider');
?>